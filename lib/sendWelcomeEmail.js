...
export async function sendPurchaseCancelEmail({
  toEmail,
  requestNo,
  cancelBy,
  cancelByRole,
  ...
  await transporter.sendMail({
    from: `"NSTREN System" <${process.env.SMTP_USER}>`,
    to: toEmail,
    subject: `Purchase Requisition Cancel- ${requestNo}`,
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:auto;padding:24px;">
        <h2 style="color:#16a34a;">Purchase Requisition Approved</h2>

        <p>Purchase Requisition has been Cancel.</p>

        <div style="background:#f9fafb;border:1px solid #e5e7eb;padding:16px;border-radius:8px;">
          <p><strong>Request No:</strong> ${requestNo}</p>
          <p><strong>Cancel By:</strong> ${cancelBy} (${cancelByRole})</p>
        </div>
...
export async function sendPurchaseRejectedEmail({
  ...
  await transporter.sendMail({
    from: `"NSTREN System" <${process.env.SMTP_USER}>`,
    to: toEmail,
    subject: `Purchase Requisition Request ${requestNo}`,
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:auto;padding:24px;">
