...
export async function GET(request) { 
  ...
  return await GetSpecificRequest("Project Director", searchParams.get("dateStart"), searchParams.get("dateEnd"), page, limit);
}
...
export async function POST(request) {
 try{
    ...
    await purchase.update({ProjectDirectorSign: body.e_sign})
     return NextResponse.json({message : `You Approve Purchase Requisition: ${id}`}, {status: 200}); 
 }catch(error){  
